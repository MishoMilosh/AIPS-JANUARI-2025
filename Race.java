...
public class Race {
    ...
    {
        SLLNode<Athlete> previous = prevWinners.getFirst();
        SLLNode<Athlete> current = currYearRunners.getFirst();
        SLLNode<Athlete> temp = null;
        double maxtime = 0;
        while(previous != null)
        {
            if(previous.element.getTime() > maxtime)
            {
                maxtime = previous.element.getTime();
            }
            previous = previous.succ;
        }
        while(current != null)
        {
            if(current.element.getTime() > maxtime)
            {
                temp = current;
                currYearRunners.delete(temp);
            }
